...
async function updateStatiProgetti() {
  try {
    ...
    // Converti "attiva" in "in corso"
    const resultAttiva = await Collaborazioniwebdesign.updateMany(
      { stato: 'attiva' },
      { $set: { stato: 'in corso' } }
    );
    console.log(`Aggiornati ${resultAttiva.modifiedCount} progetti da "attiva" a "in corso"`);

    // Converti "completata" in "terminata"
    const resultCompletata = await Collaborazioniwebdesign.updateMany(
      { stato: 'completata' },
      { $set: { stato: 'terminata' } }
    );
    console.log(`Aggiornati ${resultCompletata.modifiedCount} progetti da "completata" a "terminata"`);

    // Imposta "in corso" per progetti senza stato
    ...
    // Mostra il riepilogo degli stati finali
    ...
    console.log('\nRiepilogo stati progetti dopo aggiornamento:');
    stati.forEach(stato => {
